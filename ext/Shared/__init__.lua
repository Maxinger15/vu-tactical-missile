...
--Events:Subscribe('BundleMounter:GetBundles', function(bundles)
  ...
  --})
--end)
Events:Subscribe('Partition:Loaded', function(p_Instance)

    if p_Instance.instanceGuid == m_UpdateAgeDataGuid then
        local s_AgeData = UpdateAgeData(p_Instance)
        s_AgeData:MakeWritable()
        s_AgeData.lifetime = 50
      end
      
      if p_Instance.instanceGuid == m_SphereEvaluatorDataGuid then
        local s_SphereEvaluatorData = SphereEvaluatorData(p_Instance)
        s_SphereEvaluatorData:MakeWritable()
        s_SphereEvaluatorData.radius = 140
        s_SphereEvaluatorData.scale = Vec3(1, 1, 0.85)
      end
      
      if p_Instance.instanceGuid == m_SpawnRateDataGuid then
        local s_SpawnRate = SpawnRateData(p_Instance)
        s_SpawnRate:MakeWritable()
        s_SpawnRate.spawnRate = 1000
      end
